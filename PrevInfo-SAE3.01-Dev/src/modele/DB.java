...
public class DB {
	...
	private DB() {
		...
		try {
			...
			this.psUpdateCategorie    = connec.prepareStatement("UPDATE Categorie SET nomCategorie = ?, hMaxCategorie = ?, hMinCategorie = ? WHERE idAnnee = ? AND idCategorie = ?");
			this.psUpdateIntervenant  = connec.prepareStatement("UPDATE Intervenant SET prenom = ?, nom = ?, email = ?, hMinIntervenant = ?, hMaxIntervenant = ?, idCategorie = ? WHERE idAnnee = ? AND idIntervenant = ?");
			this.psUpdateIntervention = connec.prepareStatement("UPDATE Intervention SET nbSemainesIntervention = ?, nbGroupe = ? WHERE idAnnee = ? AND idIntervenant = ? AND idModule = ? AND idTypeCours = ?");
			this.psUpdateModule       = connec.prepareStatement("UPDATE Module SET nomModule = ?, nbSemainesModule = ?, idSemestre = ? WHERE idAnnee = ? AND idModule = ?");
			this.psUpdateSemestre     = connec.prepareStatement("UPDATE Semestre SET nbGTD = ?, nbGTP = ?, nbGCM = ?, nbGAutre = ? WHERE idAnnee = ? AND idSemestre = ?");
			this.psUpdateHeureCours   = connec.prepareStatement("UPDATE HeureCours SET heure = ? WHERE idTypeCours = ? AND idModule = ? AND idAnnee = ?");
			this.psUpdateTypeCours    = connec.prepareStatement("UPDATE TypeCours SET coefficient = ?, nomCours = ? WHERE idTypeCours = ?");
			...
			this.psDeleteIntervention = connec.prepareStatement("DELETE FROM Intervention WHERE idIntervenant = ? AND idModule = ? AND idTypeCours = ?");
			...
			this.psDeleteHeureCours   = connec.prepareStatement("DELETE FROM HeureCours WHERE idTypeCours = ? and idModule = ?");
	...
	public HashMap<String, HeureCours> getHeureCours (int idAnnee) throws SQLException{
		...
		while (rs.next()) {
			hmHeureCours.put(rs.getInt("idTypeCours")+"-"+rs.getInt("idModule"),new HeureCours(rs.getInt("idTypeCours"), rs.getInt("idModule"), rs.getDouble("heure"), rs.getInt("idAnnee")));
		}
		return hmHeureCours;
	}
